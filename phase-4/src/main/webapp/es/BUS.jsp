...
<head>
	...
	<title>Horarios :: UniBus</title>
</head>
	<body>
		<jsp:include page="header.jsp" />
		<h1 class="px-1" style="transform: translateY(-24px);">
			<span style="font-size: 0.5em;">Horario de</span>
		...
		<div class="mx-auto max-w px-1 py-2">
			<div class="grid-3 grid-2-1-1 border">
				...
				<div class="head">Detalles</div>
				<div class="head">Boleto</div>
				...
				<div class="item"><a class="btn-sm blank" href='busdetails.jsp?tid=<%= list.get(i).getTid() %>'>Mostrar</a></div>
				<div class="item"><a class="btn-sm" href='reservation.jsp?rtid=<%= list.get(i).getTid() %>&depart=<%= list.get(i).getDstation() %>
				&arrive=<%= list.get(i).getAstation() %>&ddate=<%= list.get(i).getDdate() %>&dtime=<%= list.get(i).getTime() %>'>Reservar</a></div>
				<%
					}
				%>
			</div>

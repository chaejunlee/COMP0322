...
where s.stname in (
    ...
    where dstation = a.stname
      and astation = b.stname
            and b.address not like '제주%'
      and a.address like '부산%');
...
where trid = rid and rid = prid
  and dstation = '동대구역' and astation = '영등포역'
...
where trid in (
    (select rid
    from route
    where dstation = '신경주역') -- 결과 84개 
      intersect
    (select prid
    from price
    where fee between 6000 and 6500 and Age = '성인')); -- 결과 2개 
-- 총 결과 1개 (rid = 48)
